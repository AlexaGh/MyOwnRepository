...
public class MemoryGamePanel extends JPanel {
	...
	public MemoryGamePanel(String codeBaseIn, boolean isApplet) {
		...
		try {
			for (int i = 0; i < NUM_IMAGES; i++)
				bgImageList.add(ImageLoader.getImage(new URL(codeBase + "images/" + i + "_bg.gif")));
		...
		buttonPanel = new ButtonPanel(getCardButtonList(), (Image) bgImageList.get(bgImgNum));
		ctrlPanel = new CtrlPanel(ctrlButtons, (int) buttonPanel.getPreferredSize().getWidth());
		...
		size = new Dimension((int) (buttonPanel.getPreferredSize().getWidth()),
				(int) (buttonPanel.getPreferredSize().getHeight() + ctrlPanel.getPreferredSize().getHeight()));
	...
	public ArrayList<CardButton> getCardButtonList() {
		...
		for (int i = (int) (Math.random() * numImgs) * 2; currButtonList.size() != MemoryGame.NUM_CARDS; i += 2) {
